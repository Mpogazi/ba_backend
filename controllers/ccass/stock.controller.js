...
exports.getStockOnDate = (req, res) => {
	mysql_db_confi.pool.getConnection((err, conn) => {
		...
		conn.query('SELECT code, ashare FROM ccass_stock_info WHERE ?', [req.params.date], (err, result) => {
			if (err) {
				return res.status(wrapper.STATUS_CODES.SERVICE_UNAVAILABLE).send(wrapper.wrapper_response('Error', ''));
			} else {
				res.status(wrapper.STATUS_CODES.OK).send(wrapper.wrapper_response('success', result));
...
exports.getHistorical = (req, res) => {
	mysql_db_confi.pool2.getConnection((err, conn) => {
		...
		conn.query('SELECT * FROM historical_stock_info WHERE yf_code = ?', [req.params.yf_code], (err, result) => {
			if (err) {
				return res.status(wrapper.STATUS_CODES.SERVICE_UNAVAILABLE).send(wrapper.wrapper_response('Error', ''));
			} else {
				res.status(wrapper.STATUS_CODES.OK).send(wrapper.wrapper_response('success', result));
